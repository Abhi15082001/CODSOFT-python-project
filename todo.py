...
def inputError() :
	...
	if enterTaskField.get() == "" :
		...
		messagebox.showerror('Python Error', 'Error: Please enter item name')
...
def delete() :
	...
	if len(tasks_list) == 0 :
		messagebox.showerror('Python Error', 'Error: Item list is Empty!')
		return
...
if __name__ == "__main__" :
	...
	gui.geometry("675x450")

	# create a label : Enter Your Task
	enterTask = Label(gui, text = "Enter the item name to Add Items", bg = "light green")
	...
	Submit = Button(gui, text = "Add Item", fg = "Black", bg = "Red", command = insertTask)

	# create a text area for the root
	# with lunida 13 font
	# text area is for writing the content
	TextArea = Text(gui, height = 8, width = 50, font = "lucida 18")

	# create a label : Delete Task Number
	taskNumber = Label(gui, text = "Delete Task Number", bg = "yellow")
						
	taskNumberField = Text(gui, height = 1, width = 4, font = "lucida 18")
